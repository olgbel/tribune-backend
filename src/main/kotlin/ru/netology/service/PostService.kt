...
class PostService(private val repo: PostRepository,
    ...
    suspend fun getPostsAfter(currentUser: UserModel, id: Long): List<PostResponseDto>  {
        return repo.getPostsAfter(id).map { PostResponseDto.fromModel(currentUser, currentUser, it) }
    }

    suspend fun getPostsBefore(currentUser: UserModel, id: Long): List<PostResponseDto> {
        return repo.getPostsBefore(id).map { PostResponseDto.fromModel(currentUser, currentUser, it) }
    }
